...
export function LiveCallsBand({
  ...
  return (
    ...
    >
      ...
      {/* Live call list — only when there are active calls */}
      {idle ? (
        <p className="text-muted-foreground mt-3 text-sm">
          {mockMode
            ? "Mock mode inserts calls as completed — flip TWILIO_LIVE / ELEVENLABS_LIVE to see real-time activity."
            : "When the dialer picks up a lead, it appears here. The cron ticks every 30 seconds."}
        </p>
      ) : (
      ...
      {/* Show a "nothing to listen to" hint icon in idle state */}
      {idle ? (
        <PhoneOff
          aria-hidden
          className="text-muted-foreground/30 absolute -right-3 -bottom-3 size-24"
        />
      ) : null}
    </section>
  );
