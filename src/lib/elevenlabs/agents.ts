...
const DATA_COLLECTION_FIELDS = [
  {
    ...
    description:
      ...
      "gatekeeper (the only HUMAN you spoke with was NOT the decision maker — a receptionist, front desk, or other staff who isn't the owner/manager — AND no callback time or window of ANY kind was given. Use this whenever you never got through to the decision maker, INCLUDING when that non-decision-maker declines, brushes you off, or answers the pitch on the business's behalf — you do NOT need to be explicitly refused a transfer. BUT the moment any time or window to call back is mentioned — even just when the owner is next reachable, e.g. 'before 4pm', 'after 9', 'try Monday' — use callback instead and record callback_datetime; do NOT use gatekeeper. Do NOT use it for a voicemail or a call that hung up in the first few seconds); " +
