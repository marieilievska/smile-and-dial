...
const DATA_COLLECTION_FIELDS = [
  {
    ...
    description:
      ...
      "goal_met (the call ACCOMPLISHED the campaign's goal WITH THE DECISION MAKER — the owner, or a manager who can decide, NOT a gatekeeper. For a booking/appointment campaign (e.g. inviting the owner to the webinar) this means the decision maker was actually BOOKED: you completed the booking and a real appointment/registration was created — NOT merely that you emailed the info or a receptionist agreed to 'pass it along'. For a research/survey campaign it means the owner/manager THEMSELVES answered all the questions. HARD RULE: if you only reached a gatekeeper / front desk and never the owner — even if they took the owner's email, agreed to forward it, or you sent the info by email — that is NOT goal_met, it is gatekeeper. A call that ended before the goal was actually completed is NOT goal_met either. A courtesy follow-up offer does not downgrade a goal that WAS completed. Pick goal_met ONLY when the goal was truly completed on THIS call; it then takes PRIORITY over callback); " +
      "callback (a specific time OR timeframe to call back was given — e.g. 'call me tomorrow at 3', 'try me next week', 'call after 9 to reach the owner', 'the owner's in before 4, try then'. This is a POSITIVE signal: they're open to talking, just not right now. Use this EVEN IF the person was a gatekeeper / front desk, and EVEN IF the time was for reaching someone ELSE (a receptionist telling you when the owner is in still counts) — a scheduled callback is more actionable than 'gatekeeper', so callback ALWAYS WINS over gatekeeper whenever any real time or window was given. HARD RULE: if you record a callback_datetime, the disposition MUST be callback — never gatekeeper, hung_up, or voicemail. The ONLY thing that outranks callback is goal_met. Whenever you use callback, also fill callback_datetime. Do NOT use this if the goal was already accomplished on this call — that is goal_met); " +
      ...
      "gatekeeper (the only HUMAN you spoke with was NOT the decision maker — a receptionist, front desk, or other staff who isn't the owner/manager — who could NOT connect you and did NOT refuse the offer: the owner was simply unavailable, they offered to take a message, or brushed you off without a firm 'no' — AND no callback time or window of ANY kind was given. Use this whenever you never got through to the decision maker WITHOUT a clear refusal. This ALSO covers a plain 'not now' / 'call me back later' / 'I'm busy' brush-off with NO specific time — from ANYONE, including the owner — where a retry is worthwhile but nothing got resolved (there is no separate 'call back later' option; use gatekeeper). BUT: if that non-decision-maker FIRMLY declined the offer on the business's behalf ('we're not interested', 'the owner's not interested', 'take us off your list'), use gatekeeper_not_interested instead. And the moment any time or window to call back is mentioned — even just when the owner is next reachable, e.g. 'before 4pm', 'after 9', 'try Monday' — use callback instead and record callback_datetime. Do NOT use it for a voicemail or a call that hung up in the first few seconds); " +
      "gatekeeper_not_interested (a receptionist, front desk, or staff member — NOT the owner/decision maker — who FIRMLY declined the offer on the business's behalf and doesn't want us to call again, e.g. 'we're not interested', 'the owner's not interested', 'we're all set'. Use this INSTEAD OF gatekeeper when a NON-decision-maker clearly refuses, and INSTEAD OF not_interested when the person refusing is NOT the owner/manager. If any callback time or window was given, use callback instead); " +
