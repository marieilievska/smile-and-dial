...
function buildUpdate(input: CampaignInput) {
  return {
    ...
    // a value EL can't honor. 40 on the current Business plan (2026-09-02),
    // raised from 30 on Scale (2026-08-03) and 20 on Pro before that. Shared
    // workspace-wide INCLUDING inbound, so setting a campaign to the full 40
    // leaves inbound nothing; that is deliberately the operator's call, not a
    // clamp.
    //
    ...
    // campaigns set to 40 therefore share ONE 40-call budget, and a campaign
