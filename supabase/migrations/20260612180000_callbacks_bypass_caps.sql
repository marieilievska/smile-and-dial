-- Scheduled callbacks also bypass the campaign's hourly + daily CALL-VOLUME
-- caps. Those caps pace COLD outreach; a callback is a time the prospect
-- agreed to, so it should always go through rather than be throttled behind the
-- cold-call quota. (The callback-priority queue already puts callbacks first;
-- this stops the volume caps from blocking them entirely.)
--
-- Still enforced for callbacks: the per-lead in-flight guard, owner CONCURRENCY
-- cap (real-time safety — don't double-dial an owner's lines), and SPEND caps
-- (a hard budget ceiling). Only the hourly/daily call-count throttles are
-- skipped for callbacks. Non-callback leads are completely unchanged.
--
-- Re-declares pre_call_check from 20260612170000 (which added the callback
-- calling-hours floor) with ONLY the hourly/daily cap blocks now guarded by
