-- Allow CALLBACKS to be scheduled + dialed on weekends (agreed appointments a
-- lead requested), while cold/first-touch outreach stays Mon–Fri.
--
-- The weekday gate lived inside is_within_calling_hours, so it blocked EVERY
-- weekend dial (even callbacks, which then rolled to Monday). Add an
-- `allow_weekends` argument; the dial_queue view and pre_call_check pass `true`
-- for callbacks (status='callback') and `false` for cold leads. The old 3-arg
-- version is left in place (unreferenced) to avoid a risky drop.
...
comment on function public.is_within_calling_hours(text, time, time, boolean) is
  ...
  '(allow_weekends OR it is a weekday Mon–Fri). Callbacks pass allow_weekends=true '
  'so an agreed weekend appointment can dial; cold outreach passes false.';

-- dial_queue view: callbacks now allow weekends (allow_weekends=true), cold leads
-- stay weekday-only. Otherwise identical to 20260621120100.
...
-- pre_call_check: re-declared verbatim from 20260702130000 (the pacing version),
-- changing ONLY the is_within_calling_hours call to pass allow_weekends =
-- (status = 'callback') so callbacks can pass the gate on weekends.
...
comment on function public.pre_call_check is
  ...
  'calling window (kept inside an 08:00-21:00 local floor), MAY dial on weekends, '
  'and bypass the pacing interval + hourly/daily volume caps, since they are '
  'agreed appointments; concurrency + spend caps and the in-flight guard still '
  'apply to them. Cold/retry leads are weekday-only and paced by '
  'dial_interval_seconds. Spend-cap windows are bound to America/New_York.';
