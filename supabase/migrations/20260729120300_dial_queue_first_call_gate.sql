-- ---------------------------------------------------------------------------
-- dial_queue: make the local-match ranking actually bind.
--
-- 20260729120200 added dest_rank / local_match_rank but sorted them AFTER
-- queue_order, on the assumption that first-call leads all tie there with a
-- null next_call_at. They do not: pre_call_check stamps next_call_at =
-- now() + 5 minutes on every lead it blocks on a cap, so within two days of
-- dialling only 3 of 9,475 dialable leads still had a null and dial_queue had
-- ZERO. The ranking was computed and never consulted.
--
-- This migration moves the two keys ahead of queue_order and folds an explicit
-- first-call gate (retry_counter = 0) into the rank columns themselves, so a
-- genuine retry collapses to 0 on both and keeps its time-ordered slot. The
-- gate lives in the columns rather than in the ORDER BY because PostgREST
-- clients can only order by plain columns, and the dialer replaces a view's
-- ORDER BY with its own .order() calls.
--
-- Reproduced in full from 20260729120200_dial_queue_local_match.sql. Every
-- DOUBLE CALL, TOGGLE, and LOCAL MATCH branch is preserved.
--
...
-- ENTIRE definition, copied from the latest migration that defines it.
-- ---------------------------------------------------------------------------
...
-- LOCAL MATCH: the two rank keys sort BEFORE queue_order, and carry their own
-- first-call gate instead of relying on one.
--
-- The first attempt (migration 20260729120200) put them AFTER queue_order,
-- reasoning that every never-called lead has next_call_at null so they would
-- all tie there and the ranks would break the tie. That is wrong in practice:
-- pre_call_check stamps next_call_at = now() + 5 minutes on every lead it
-- blocks on a cap, so after two days of dialling only 3 of 9,475 dialable
-- leads still had a null -- and dial_queue had ZERO. The keys were computed
-- and then never consulted, because millisecond-precision timestamps never tie.
--
-- So the gate is now explicit: a genuine retry (retry_counter > 0) collapses to
-- 0 on BOTH rank columns, keeping the time-ordered slot the retry cycle gave
-- it. That preserves today's behaviour exactly -- a retry's next_call_at is the
-- hour it fell due, which is already older than a cap-bumped cold lead's
-- now()+5min, so retries already sorted first and still do. First-call leads
-- (retry_counter = 0) are the only rows the ranks reorder.
--
-- Folding the gate INTO the columns (rather than a CASE in the ORDER BY) is
-- deliberate: PostgREST clients can only order by plain columns, and the dialer
-- overrides a view's ORDER BY with its own .order() calls.
order by
  q.dial_priority,
