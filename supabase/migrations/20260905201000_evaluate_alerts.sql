-- Alerting for silent stops, part 2: the rule evaluator.
--
-- `evaluate_alerts()` runs every 5 minutes under pg_cron (`alerts-evaluate`)
-- and turns the conditions that used to fail silently into in-app
-- `notifications` for the campaign owner (or, for account-wide faults, every
-- active admin). Each rule dedupes through alert_fire() so it fires at most
-- once per ref per period; the periods are chosen so a rhythm (an hourly cap)
-- stays quiet and a fault (a stalled dialer) re-alerts if it persists.
--
-- Rules (kind = rule name, ref_table/ref_id as noted, period):
--   a. cap_hit_daily            campaigns   once per ET day
--   b. cap_hit_hourly           campaigns   once per 6 h
--   c. dialer_stalled           campaigns   once per 2 h
--   d. pool_exhausted           campaigns   once per 6 h
--   e. cron_missed              cron job    once per 6 h  (all admins)
--   f. placement_storm          campaigns   once per 1 h
--   g. callbacks_piling_paused  campaigns   once per 24 h
--   h. integration_missing      campaigns   once per 24 h
--   i. meta_sync_failed         profiles    once per 24 h
--   j. credit_read_failed       (global)    once per 2 h  (all admins)
--
-- Every rule sits in its own exception block: one rule erroring (a renamed
-- column, a missing extension) is recorded in the returned jsonb under
-- "errors" and never stops the others. `select public.evaluate_alerts();`
-- from the SQL editor shows what fired and what broke.
--
-- Kept cheap on purpose: each rule is one query per active campaign (two
-- today) over a short time window, on the composite indexes added in
-- 20260905200000. The only heavier read is the dial_queue EXISTS in rule c,
-- and it runs only once a stall is already likely (inside the window, no
-- call in 15 min, and a fault signal in the heartbeats).
--
-- No execute grants: pg_cron runs this as postgres; nothing user-facing
-- calls it (20260905170000 closed EXECUTE by default).

create extension if not exists pg_cron;

-- ---------------------------------------------------------------------------
-- How often a pg_cron schedule fires, in minutes — for the simple forms our
-- jobs actually use. Anything unrecognised is treated as a daily job (the
-- caller maps >= 1440 to a 26 h grace, per the rule). Pure and immutable.
--   '* * * * *'        -> 1        '*/5 * * * *'   -> 5
--   '30 * * * *'       -> 60       '0 */6 * * *'   -> 360
--   '30 3 * * *'       -> 1440     '30 seconds'    -> 1
--   '*/30 * * * * *'   -> 1  (six-field seconds form)
-- ---------------------------------------------------------------------------
create or replace function public.cron_schedule_minutes(in_schedule text)
returns integer
language sql
immutable
...
as $$
  select case
    when in_schedule ~ '^\s*\d+\s+seconds?\s*$' then 1
    when in_schedule ~ '^\s*\*(/\d+)?(\s+\*){5}\s*$' then 1
    when in_schedule ~ '^\s*\*(\s+\*){4}\s*$' then 1
    when in_schedule ~ '^\s*\*/\d+(\s+\*){4}\s*$'
      then (regexp_match(in_schedule, '^\s*\*/(\d+)'))[1]::integer
    when in_schedule ~ '^\s*\d+\s+\*(\s+\*){3}\s*$' then 60
    when in_schedule ~ '^\s*\d+\s+\*/\d+(\s+\*){3}\s*$'
      then 60 * (regexp_match(in_schedule, '^\s*\d+\s+\*/(\d+)'))[1]::integer
    else 1440
  end;
$$;

comment on function public.cron_schedule_minutes(text) is
  'Minutes between runs for the simple cron forms our jobs use (every N '
  'minutes / hours, or a fixed daily time). Unrecognised schedules read as '
  'daily (1440). Used by evaluate_alerts() to spot a job that stopped.';
  ...
  begin
    ...
    loop
      ...
      v_needs_calendly :=
        ...
        or nullif(btrim(v_c.calendly_event_id), '') is not null
...
comment on function public.evaluate_alerts() is
  'Alert evaluator, every 5 min via pg_cron (alerts-evaluate). Inserts '
  'notifications for silent dialing stops (caps, stalled dialer, exhausted '
  'pool, missed cron jobs, placement storms, callbacks piling on a paused '
  'campaign, missing integrations, Meta sync failures, credit-read outages), '
  'deduped per (rule, ref) through alert_fire(). Returns {fired, errors}.';

-- Idempotent schedule.
select cron.unschedule(jobid)
from cron.job
where jobname = 'alerts-evaluate';
