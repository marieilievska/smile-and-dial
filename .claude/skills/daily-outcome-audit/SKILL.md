...
**Core principle:** every outcome has an _objective_ ground-truth signal that beats the AI's guess (a booking row, a termination reason, a self-request in the transcript). Judge against the signal, not the label.
...
**Fast path (all at once):** `node scripts/triage.js [YYYY-MM-DD]` does a read-only pass and prints a per-outcome **scorecard**, flags the ~40 calls whose label contradicts a signal the AI already recorded, and writes a **read list** (`_out-triage-<date>.txt` — the flagged calls + all dnc + all goal_met) plus a **suggested relabel map** (`map-triage-<date>.json`, high-confidence structural only, NEVER auto-applied). It also appends one line per day to `scorecard.jsonl` and **flags drift** — any watched metric (`not_interested`-with-dm≠yes share, `ai_receptionist` share, callback share, connect rate, goal_met/dnc counts) that moved beyond its threshold vs the trailing-median baseline of prior audited days (a same-day sign of an agent/campaign change). Read the read-list, then jump to step 5 (fix) and 6 (harden). Use `audit-outcome.js <outcome>` to drill into one outcome.

1. **Credit check first** — `node scripts/credit-check.js`. `ai_error` = ElevenLabs ran out of credits; a spike means calls are failing _right now_. Fix billing before anything else. (Only Marija can top up.)
...
4. **Reconcile goal_met** — `node scripts/reconcile-bookings.js` cross-checks real Calendly bookings ↔ `goal_met` in BOTH directions (false wins _and_ real bookings hiding under the wrong label).
...
| ----------------------------------------------- | ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `goal_met`                                      | real Calendly booking (`leads.calendly_event_uri` OR a `scheduled` `calendly_events` row + `book_appointment` tool) | gatekeeper email marked as a win; real bookings mislabeled `gatekeeper`; phantom bookings (tool fired w/o a yes); a merged inbound lead that lost its link |
| `dnc`                                           | the **person themself** asked to stop, unprompted                            | agent _offered_ removal ("make sure we don't bug you again") → "sure" got marked dnc; a closed business → `invalid_number`; "get me out of here" ≠ a request |
| `ai_receptionist`                               | the bot **says** it's an AI, in its own words                                | a person _asking_ "are you an AI agent?"; a bot that transfers to a real person — read every one                                                          |
...
| `(null)` on a **completed** call                | should NEVER exist — the webhook always buckets now                          | pre-fix strandings / an EL quota-kill whose reason lands _after_ the webhook → re-fetch EL + reclassify (see step 1b); delete no-conversation quota-kills |

Full recipes: **`outcome-playbook.md`**. Fix mechanics + safety: **`fix-patterns.md`**.

**Triage flags (what `triage.js` surfaces):** `not_interested` with `decision_maker_reached ≠ "yes"` → likely `gatekeeper_not_interested`; `gatekeeper_not_interested` with `dm="yes"` → read; `goal_met` with no booking → false win; `callback` with no time and no `callbacks` row → stranded; `dnc` where an **agent** offered removal → agent-manufactured; an agent removal offer on a call that is **not** `dnc` → did they accept? (promised no more calls); a `dnc_entries` row citing a call that isn't dnc-family → list/lead/label disagree; sampled `voicemail` with ≥2 human replies → human-then-mailbox; null outcome on a completed call → stranded. Always-read: every `dnc`, `goal_met` and `ai_receptionist`. The suggested map never includes `outcome_source='manual'` rows. `decision_maker_reached` is a **string enum** (`yes`/`no`/`unknown`/absent), NOT a boolean.
...
- Live agents via `ELEVENLABS_API_KEY` (the Smile & Dial workspace — separate from the old multi-department one as of 2026-08-18). One active agent (checked 2026-09-11): **Daily HireAI Webinar** `agent_2401m249x77dej8vdvy2dh56r394`, campaign "Daily HireAI Webinar" `d20817b8…`. It is **externally managed**: its prompt AND its Procedures (Gatekeeper Handling, Handle Pushback and Objections, Book Webinar Seat, Schedule Callback) are edited by Marija in the ElevenLabs dashboard, and procedure bodies can't be read via the API. For a behaviour fix, hand her the exact wording — don't PATCH it. Tool definitions stay ours.
...
- **Never create/cancel real Calendly registrations to "test".** Cancelling a shared webinar (group) event drops _every_ registrant. Booking is additive/OK; cancelling is off-limits.
